...
import { BUA_DEFAULT_CHANNEL } from './consts';
import { INIT_SCRIPT } from './consts';
...
export class BrowserSession {
	...
	constructor(public browserProfile: BrowserProfile) {}

	setConnection({ wssUrl, cdpUrl }: { wssUrl?: string; cdpUrl?: string }) {
		if (wssUrl) this.wssUrl = wssUrl;
		if (cdpUrl) this.cdpUrl = cdpUrl;
	}

	async newContext(options?: BrowserContextOptions): Promise<this> {
		if (!this.browser) throw new BrowserSessionError('Browser is not initialized');
	...
	private async _setupBrowserViaPassedObjects(): Promise<void> {
		this.browserContext =
			(this.agentCurrentPage && this.agentCurrentPage.context
				? this.agentCurrentPage.context()
				: undefined) ||
			this.browserContext ||
			undefined;
		if (this.browserContext) {
		...
		let browserFromContext: any = undefined;
		if (this.browserContext && typeof this.browserContext.browser === 'function') {
	...
	private async _setupBrowserViaBrowserPid(): Promise<void> {
		...
		console.info(
			`🌎 Connecting to existing local browser process: browser_pid=${pid} on ${this.cdpUrl}`,
		);
		this.browser =
			this.browser ||
	...
	private async _setupBrowserViaWssUrl(): Promise<void> {
		...
		this.browser =
			this.browser ||
			(await this.playwright.connect(this.wssUrl, this.browserProfile.toConnectOptions()));
		this.browserProfile.keepAlive = true;
	}
	...
	private async _setupNewBrowserContext(): Promise<void> {
		...
		if (!this.browserContext) {
			console.info(
				`🌎 Launching local ${this.playwright.name() || 'chromium'} ${this.browserProfile.channel?.toLowerCase() || BUA_DEFAULT_CHANNEL} browser with user_data_dir=${this.browserProfile.userDataDir || 'None (incognito)'}`,
			);
			if (!this.browserProfile.userDataDir) {
				this.browser =
					this.browser || (await this.playwright.launch(this.browserProfile.toLaunchOptions()));
				this.browserContext = await this.browser?.newContext(
					this.browserProfile.toBrowserContextOptions(),
				);
			} else {
				...
				try {
					this.browserContext = await this.playwright.launchPersistentContext(
					...
					try {
						const testBrowser = await this.playwright.launch(this.browserProfile.toLaunchOptions());
					...
					const driver = this.playwright.name ? this.playwright.name().toLowerCase() : 'chromium';
					const browserChannel = this.browserProfile.executablePath
						? this.browserProfile.executablePath
								...
								.replace('.exe', '')
								.toLowerCase()
						: (this.browserProfile.channel || BUA_DEFAULT_CHANNEL).toLowerCase();
					console.error(
		...
		if (this.browser) {
			const connectionMethod = this.cdpUrl ? 'CDP' : this.wssUrl ? 'WSS' : 'Local';
		...
		if (this.browserProfile.stealth && typeof this.playwright !== 'function') {
			console.warn(
	...
	private async _setupViewportSizing(): Promise<void> {
		...
		if (
			permissions &&
			this.browserContext &&
			typeof this.browserContext.grantPermissions === 'function'
		) {
			try {
		...
		try {
			if (
				...
				this.browserContext &&
				typeof this.browserContext.setExtraHTTPHeaders === 'function'
			) {
		...
		try {
			if (
				geolocation &&
				this.browserContext &&
				typeof this.browserContext.setGeolocation === 'function'
			) {
		...
		let page = lastPage || (await this.browserContext.newPage());
	...
	private async _setupCurrentPageChangeListeners(): Promise<void> {
		if (!this.browserContext) return;
		const pages =
			typeof this.browserContext.pages === 'function' ? this.browserContext.pages() : [];
		let foregroundPage = pages.length ? pages[0] : await this.browserContext.newPage();
		...
		const _BrowserUseonTabVisibilityChange = (source: Record<string, any>) => {
			...
			const oldUrl = typeof oldForeground.url === 'function' ? oldForeground.url() : '';
			const newUrl = typeof newPage.url === 'function' ? newPage.url() : '';
			const agentUrl =
				agentCurrentPage && typeof agentCurrentPage.url === 'function'
					? agentCurrentPage.url()
					: '';
			if (oldUrl !== newUrl) {
				console.info(
	...
	async saveStorageState(path?: string): Promise<void> {
		...
		if (path) {
			await fs.writeFile(path, JSON.stringify(storageState, null, 4));
			console.info(`💾 Saved storage state to ${path}`);
			return;
		}
		if (typeof this.browserProfile.storageState === 'string') {
			await fs.writeFile(this.browserProfile.storageState, JSON.stringify(storageState, null, 4));
	...
	async loadStorageState(path?: string): Promise<void> {
		...
		if (storagePath) {
			try {
				const storageText = await fs.readFile(storagePath, 'utf8');
		...
		if (this.browserProfile.cookiesFile) {
			const cookies = JSON.parse(await readFile(this.browserProfile.cookiesFile, 'utf-8'));
			...
			console.info(`🍪 Loaded ${cookies.length} cookies from ${this.browserProfile.cookiesFile}`);
		}
	}

	private async _setupPlaywright() {
		if (this.browserProfile.stealth) {
			this.playwright = (await import('patchright')).chromium as BrowserType;
			...
			this.playwright = (await import('playwright')).chromium;
		}
	}
