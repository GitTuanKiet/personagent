...
import type { BrowserContextOptions, Page, BrowserType, Browser } from 'patchright';
import { DEFAULT_CHANNEL, INIT_SCRIPT } from './const';
import { BrowserError } from './interface';
import { BrowserSessionAbstract, type BrowserSessionAbstractArgs } from './abstract';
...
export class BrowserSession extends BrowserSessionAbstract {
	...
	private async _setupBrowserViaWssUrl(): Promise<void> {
		...
		this.browser =
			this.browser ||
			(await this.playwright.connect(
				this.browserProfile.wssUrl,
				this.browserProfile.toConnectOptions(),
			));
		this.browserProfile.keepAlive = true;
	}
		...
		if (!this.browserContext) {
			if (!this.browserProfile.userDataDir) {
				...
				try {
					...
					const driver = this.playwright.name();
	...
	private async _setupPlaywright() {
		...
		if (this.browserProfile.stealth) {
			// @ts-ignore
			this.playwright = (await import('patchright')).chromium;
			...
			try {
				...
				console.warn('Error importing playwright:', e);
			}
		}
	}
