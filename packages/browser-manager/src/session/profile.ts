...
import { getDisplaySize, isDocker } from './utils';
import type { IBrowserProfile, LaunchPersistentContextOptions } from './interface';
import type {
...
export class BrowserProfile implements IBrowserProfile {
	...
	detectDisplayConfiguration() {
		...
		if (this.headless) {
			this.viewport = this.viewport === null ? this.viewport : this.windowSize || this.screen;
