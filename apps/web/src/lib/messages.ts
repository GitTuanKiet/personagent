import { useExternalMessageConverter, ToolCallContentPart } from '@assistant-ui/react';
...
export const getMessageType = (message: Record<string, any>): string => {
	if (Array.isArray(message.id)) {
		...
		if (lastItem.startsWith('HumanMessage')) {
			...
			return 'tool';
		} else if (lastItem.startsWith('BaseMessage') || lastItem.startsWith('SystemMessage')) {
			return 'system';
		}
	}
...
function getMessageContentOrThrow(message: unknown): string {
	...
	if (
		...
		(!castMsg.kwargs || !castMsg.kwargs?.content || typeof castMsg.kwargs?.content !== 'string')
	) {
...
export const convertLangchainMessages: useExternalMessageConverter.Callback<BaseMessage> = (
	message,
): Message | Message[] => {
