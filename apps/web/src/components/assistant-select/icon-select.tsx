...
const prettifyIconLabel = (iconName: string) => {
	iconName = iconName.replace('Icon', '').replace('icon', '').replace('Lucide', '');
...
export function IconSelect({
	...
	const filteredIcons: Array<{ value: string; label: string }> = useMemo(() => {
		const iconItems = uniq(
			Object.keys(Icons).filter((name) => name.toLowerCase().includes(searchTerm.toLowerCase())),
		);
		return uniqBy(
	...
	const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
		...
		if (scrollPercentage > 0.7 && startIndex + WINDOW_SIZE < filteredIcons.length) {
			setStartIndex(Math.min(startIndex + BUFFER_SIZE, filteredIcons.length - WINDOW_SIZE));
		...
		>
			<DropdownMenuTrigger className="w-full" asChild disabled={allDisabled}>
				{hasSelectedIcon ? (
					...
					<Button disabled={allDisabled} variant="outline" className="flex justify-between">
				...
				<div className="overflow-y-auto max-h-[420px]" onScroll={handleScroll}>
					...
					{uniq(filteredIcons)
						...
						?.map((iconItem) => (
							...
							>
								<span style={{ color: iconColor }}>
									{React.createElement(Icons[iconItem.value as KeyofIcons] as React.ElementType)}
								</span>
								{iconItem.label}
							</DropdownMenuItem>
						))}
					{startIndex + WINDOW_SIZE < filteredIcons.length && (
						<div className="py-2 text-center text-sm text-gray-500">Scroll down for more icons</div>
					)}
				</div>
			</DropdownMenuContent>
		</DropdownMenu>
