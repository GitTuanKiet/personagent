...
import { ALL_MODEL_NAMES, ALL_MODELS, LANGCHAIN_USER_ONLY_MODELS } from '@/lib/models';
import { Dispatch, SetStateAction, useCallback, useEffect, useState } from 'react';
import { ModelConfigPanel } from './model-config-pannel';
import { IsNewBadge } from './new-badge';
import { cn } from '@workspace/ui/lib/utils';
import { CustomModelConfig, ModelConfigurationParams } from '@/lib/models';
...
function CommandModelItem({
	...
	return (
		<CommandItem value={model.name} onSelect={handleModelChange} className="flex items-center">
			<Check
				className={cn(
					'mr-1 size-4',
			...
			{openConfigModelId === model.name ? (
				<ModelConfigPanel
					...
					modelConfig={config}
					isOpen={true}
					onOpenChange={(open) => setOpenConfigModelId(open ? model.name : undefined)}
...
export default function ModelSelector({
	...
	const allAllowedModels = ALL_MODELS.filter((model) => {
		...
		if (model.name.includes('azure/') && process.env.NEXT_PUBLIC_AZURE_ENABLED === 'false') {
			return false;
		}
		if (model.name.includes('gemini-') && process.env.NEXT_PUBLIC_GEMINI_ENABLED === 'false') {
			return false;
		}
		if (model.name.includes('ollama-') && process.env.NEXT_PUBLIC_OLLAMA_ENABLED === 'false') {
			return false;
		}

		if (model.name.includes('groq/') && process.env.NEXT_PUBLIC_GROQ_ENABLED === 'false') {
			return false;
		}
	...
	const selectedModelLabel = ALL_MODELS.find((m) => m.name === modelName)?.label || modelName;
	const isSelectedModelNew = ALL_MODELS.some((m) => m.name === modelName && m.isNew);

	const azureModelGroup = allAllowedModels.filter((m) => m.config.provider === 'azure_openai');
	const openaiModelGroup = allAllowedModels.filter((m) => m.config.provider === 'openai');
	const ollamaModelGroup = allAllowedModels.filter((m) => m.config.provider === 'ollama');
	const anthropicModelGroup = allAllowedModels.filter((m) => m.config.provider === 'anthropic');
	const genAiModelGroup = allAllowedModels.filter((m) => m.config.provider === 'google-genai');
	const fireworksModelGroup = allAllowedModels.filter((m) => m.config.provider === 'fireworks');
	const groqModelGroup = allAllowedModels.filter((m) => m.config.provider === 'groq');
		...
		<Popover open={open} onOpenChange={setOpen}>
			...
			>
				<div className="flex items-center pr-2 truncate">
					<NextImage alt="Model icon" src={LLMIcon} width={14} height={14} className="mr-2" />
