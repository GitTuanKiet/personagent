			...
			try {
				...
				const addAttachmentPromises = files.map(async (file) => {
					if (
						...
						toast.error('Incompatible file type', {
							description: (
								<div className="flex flex-col gap-1 text-pretty">
									<p>This file {file.name} is not supported.</p>
									<p>
										Received type <span className="font-mono">{file.type}</span>. Must be one
										of:{' '}
									</p>
									<p className="font-mono text-wrap">{attachmentAccept.split(',').join(', ')}</p>
								</div>
							),
							duration: 5000,
						});
				...
				console.error(e);
				toast.error('Error', {
					description: 'Failed to add attachment. This is likely due to an incompatible file type.',
					duration: 5000,
				});
			}
		} else {
			toast.error('Drag and drop disabled', {
				description: 'Drag and drop is disabled in this mode. Please try again later.',
				duration: 5000,
			});
		}
	};
