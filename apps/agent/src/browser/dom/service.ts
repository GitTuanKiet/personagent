...
import { DOMBaseNode, DOMElementNode, DOMTextNode, DOMState } from './views.js';
...
export class DomService {
	...
	async getCrossOriginIframes(page: Page): Promise<string[]> {
		const hiddenFrameUrls: string[] = await page
			...
			.evaluateAll((elements) =>
				elements.map((e) => ('src' in e ? (e.src as string) : '')).filter(Boolean),
			);
	...
	private async _constructDomTree(evalPage: RecordData): Promise<[DOMElementNode, SelectorMap]> {
		const jsNodeMap = evalPage['map'] as RecordData;
	...
	private _parseNode(nodeData?: RecordData): [DOMBaseNode | null, string[]] {
